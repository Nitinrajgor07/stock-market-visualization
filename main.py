...
def render_sector_index(sector_key: str, widget_key: str):
    ...
    with _rc:
        if st.button("🔄 Refresh", key=f"ri_{widget_key}", use_container_width=True):
...
<div class="topbar">
  <div class="topbar-left">
    <span class="topbar-title">📈 Market</span>
    ...
    for tkr, name in filtered_wl:
        ...
        with chart_col:
            if st.button("📊" if not is_expanded else "✕",
                ...
                st.rerun()
        with b_col:
            if st.button("🟢 B", key=f"wlb_{tkr}", use_container_width=True, type="primary"):
                st.session_state.order_ticker = tkr
                st.session_state.order_action = "BUY"
                st.session_state.active_tab   = "orders"
                st.rerun()
        with s_col:
            if st.button("🔴 S", key=f"wls_{tkr}", use_container_width=True, type="secondary"):
                st.session_state.order_ticker = tkr
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with b_col:
        do_buy  = st.button("🟢 BUY", key="exec_buy",  type="primary",   use_container_width=True)
    with s_col:
        do_sell = st.button("🔴 SELL", key="exec_sell", type="secondary", use_container_width=True)
    ...
    with port_tab2:
        ...
        if len(sell_trades) == 0:
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px;">
              <div style="background:{CARD_BG};border:1px solid {wr_color}44;border-radius:12px;
              ...
              <div style="background:{CARD_BG};border:1px solid {pnl_color}44;border-radius:12px;
              ...
              <div style="background:{CARD_BG};border:1px solid {pf_color}44;border-radius:12px;
              ...
              <div style="background:{CARD_BG};border:1px solid #3b82f644;border-radius:12px;
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.show_add_balance:
        st.markdown(f"""
        <div style="background:{CARD};border:1px solid {G};border-radius:12px;
    ...
    st.markdown("""
    <div style="background:#1a1d27;border:1px solid #2a2d3a;border-radius:12px;padding:20px 24px;">
    ...
    if st.session_state.show_pnl_report:
        ...
        if not st.session_state.pt_history:
            ...
            st.markdown(f"""
            <div style="background:{CARD};border:1px solid {BDR};border-radius:12px;
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_summary:
        ...
        if all_news:
            ...
            <div style="background:{overall_bg};border:2px solid {overall_color};
                        border-radius:14px;padding:18px 22px;margin-bottom:16px;text-align:center;">
    ...
    with tab_defence:
        ...
        <div style="background:linear-gradient(135deg,#0f1f0a,#0a1520);
                    border:1px solid {OLIVE}55;border-radius:14px;
        ...
        with def_r2:
            if st.button("🔄 Refresh", key="def_refresh", use_container_width=True):
        ...
        if def_news:
            ...
            if big_orders:
                ...
                for n in big_orders:
                    ...
                    st.markdown(f"""
                    <div style="background:linear-gradient(135deg,#1a1200,#1a1d27);border:1px solid {SAFFRON}55;border-radius:12px;padding:14px 16px;margin-bottom:8px;border-left:4px solid {SAFFRON};">
            ...
            if other_orders:
                ...
                for n in other_orders:
                    ...
                    st.markdown(f"""
                    <div style="background:{CARD};border:1px solid {OLIVE}33;border-radius:12px;padding:13px 16px;margin-bottom:7px;border-left:4px solid {OLIVE};">
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="background:linear-gradient(135deg,#0d1117 0%,#1a1d27 100%);
                border:1px solid {BDR};border-radius:14px;padding:20px 22px;margin-bottom:18px;">
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="font-size:2rem;">🤖</div>
        <div>
...
⚠️ Disclaimer: Sirf educational analysis. Investment advice nahi."""
    ...
    if st.session_state.ai_market_result:
        ...
        st.markdown(f"""
        <div style="background:{mood_bg};border:1px solid {mood_c};border-radius:12px;
...
if tab == "defence":
    ...
    with t4:
        ...
        with ri_c2:
            if st.button("🔄 Refresh", key="ri_refresh", use_container_width=True):
